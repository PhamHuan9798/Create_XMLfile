...
namespace JSON
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int x = 0;
            foreach (DanhSach DS in List)
            {
                str += "<HocSinh>\n" + "   <ID>" + List[x].ID + "</ID>\n" + "   <HoTen>" + List[x].HoTen + "</HoTen>\n" + "   <Tuoi>" + List[x].Tuoi + "</Tuoi>\n" + "   <GioiTinh>" + List[x].GioiTinh + "</GioiTinh>\n" + "</HocSinh>\n";
                x++;
            }
            str +="</DanhSachHocSinh>";
            Console.WriteLine(str);
            File.WriteAllText(@"C:\Users\Administrator\Desktop\DanhSach1.xml", str);
